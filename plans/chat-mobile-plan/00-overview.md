...
7. **OS push notifications via Expo push** delivered through a relay-side fanout: host-service emits a new outbound `push:lifecycle` message on its existing tunnel WS to `apps/relay`; relay looks up registered Expo push tokens for the org+user in Upstash KV (relay's existing directory infra) and calls the Expo Push API. Mobile registers/de-registers tokens against the relay using the same JWT bearer it uses for chat tRPC. v2 minimal scope: `Stop` and `PermissionRequest` events only — no preferences, no ringtones, no Time-Sensitive interruption, no in-app activity center. Deep-link routing on tap is specified by `09-uc-nav.md` UC-NAV-05; the wire-level design lives at `11-technical-requirements/07-notifications.md`. This is a mobile-specific need that has no desktop analog.
