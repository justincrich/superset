...
**Test Steps:**
1. **Phase 0 pre-gate.** Confirm `14-token-migration-audit.md` ACs all check (ember tokens in `global.css`, Geist loading, manifest amended, visual regression evidence attached to the `chat-mobile-token-migration` PR). If any AC fails, Phase 0 is incomplete and Storybook walk does not begin.
