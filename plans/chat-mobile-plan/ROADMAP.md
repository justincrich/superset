...
- **Vibe relocked to ember (2026-05-22)** per [`14-token-migration-audit.md`](14-token-migration-audit.md). Phase 0 of this sprint rewrites `apps/mobile/global.css` + `apps/mobile/lib/theme.ts` against `designs/tokens/tokens.css` (warm-neutral surface ramp `#151110`/`#1a1716`/`#252220`, ember accent `#e07850`, Geist + Geist Mono, 8-step spacing, 6-step radius, full status palette). The earlier RNR cool-neutral lock is superseded.

#### Pixel-Perfect Workflow

0. **Phase 0 — Token Migration Audit** (NEW, 2026-05-22): single PR `chat-mobile-token-migration` against `chat-mobile-ui-elements` rewriting `apps/mobile/global.css` + `apps/mobile/lib/theme.ts` to ember per [`14-token-migration-audit.md`](14-token-migration-audit.md). Sub-phases 0a–0g (snapshot baseline, token rewrite, font wiring, Storybook re-verify, visual regression, manifest amendment, PR + review). Phase 0 closing gate (10 ACs in `14-token-migration-audit.md`) MUST pass before step 1.
1. `/pixel-perfect:build --platform mobile-ios` (and `--platform mobile-android`) — **plan phase**: pixel-perfect reads `plans/chat-mobile-plan/` (PRD + UCs + tech requirements + component organization addendum + Phase 0 audit output) and the Component Inventory below, then writes its atoms/molecules/compose plan to the manifest
...
- **Palette delta (mobile cool-neutral vs desktop warm-ember) — RESOLVED 2026-05-22 via Path A.** Mobile migrates to the desktop ember warm palette as canonical per [`14-token-migration-audit.md`](14-token-migration-audit.md). Sprint 01 Phase 0 is the migration vehicle; pixel-perfect's plan/atoms/molecules/compose phases consume the ember token set. The prior `apps/mobile/global.css` cool-neutral lock is superseded; the prior `constraints.preserve_theme` in `apps/mobile/design/manifest.json` is amended per audit §6.
