...
- `expo-router` — navigation; new routes under the **(chat) tab** for NAV-defined surfaces (v2.0.0 project-first model): `(authenticated)/(chat)/_layout.tsx` (tab + Stack root), `(authenticated)/(chat)/index.tsx` (flat recency-sorted sessions list — UC-NAV-01, UC-NAV-07, UC-NAV-08), `(authenticated)/(chat)/[sessionId].tsx` (chat view; `useChatTunnel` opens the lazy relay tunnel on mount per UC-NAV-05), `(authenticated)/(chat)/project-picker.tsx` (modal sheet — UC-NAV-08, renders only when org has ≥2 projects), `(authenticated)/(chat)/filter.tsx` (`SessionFilterSheet` modal — UC-NAV-08), `(authenticated)/(chat)/new-chat.tsx` (workspace-picker sheet scoped to the selected project — UC-NAV-04), and `(authenticated)/(chat)/[sessionId]/plan-review/[planId].tsx` (UC-PAUSE-03 pushed route). The v1.x host-picker route is **removed** in v2.0.0 (host is row metadata, not a top-level filter). Workspace details remain in the existing `(home)/workspaces/[id]/...` route tree (out of scope for this PRD).
