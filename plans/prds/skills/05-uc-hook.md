...
Sprint 2 of the initiative. Lefthook-based pre-commit and pre-push gate that runs strict checks for wrapped agents and short-circuits for humans. **Status: 🔵 Planned — PR `skills-hooks`.** Builds on Sprint 1 (tsgo) — the gate runs `bun run typecheck` and depends on the typechecker being fast enough to be usable.

## Change gating & blast radius

> The detailed gating fields will be filled in by PR `skills-hooks` when it opens. This section is a placeholder so reviewers know what to look for in the implementation PR.

- **Scope of this change**: TBD — populated by `skills-hooks`
- **Default behavior after merge**: TBD (likely on for wrapped agents, skipped for humans by `SUPERSET_AGENT_ID` check)
- **Containment mechanism**: TBD (lefthook phase-level skip; secondary `SUPERSET_AGENT_GATE_ENABLED` env var)
- **One-line opt-out for an individual developer**: TBD
- **One-line rollback for the whole team**: TBD
- **What this change cannot break**: Non-agent workflows; CI; existing `git push` flows for humans
- **Verification before merge**: see [`ROADMAP.md` § Sprint 02](./ROADMAP.md#sprint-02-agent-gated-commit-hooks) Test Steps
