...
Sprint 1 of the initiative. Replaces `tsc` with `@typescript/native-preview` (tsgo) across the monorepo for fast typechecking. **Status: 🔵 Planned — PR `skills-tsgo`.** The acceptance criteria below describe the developer surface this PR introduces — verified by invoking the typecheck command on the monorepo after merge.

## Change gating & blast radius

> The detailed gating fields will be filled in by PR `skills-tsgo` when it opens. This section is a placeholder so reviewers know what to look for in the implementation PR.

- **Scope of this change**: TBD — populated by `skills-tsgo`
- **Default behavior after merge**: TBD
- **Containment mechanism**: TBD (likely `FORCE_TSC=1` env var rollback path via `scripts/typecheck.sh`)
- **One-line opt-out for an individual developer**: TBD
- **One-line rollback for the whole team**: TBD
- **What this change cannot break**: Runtime code — this PR only touches build tooling
- **Verification before merge**: see [`ROADMAP.md` § Sprint 01](./ROADMAP.md#sprint-01-tsgo-migration) Test Steps
