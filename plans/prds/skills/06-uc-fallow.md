...
Sprint 3 of the initiative. **🟡 PROPOSED — PR `skills-fallow`. Scope outline below; concrete acceptance criteria locked in Sprint 3 planning kickoff.**

## Change gating & blast radius

> The detailed gating fields will be filled in by PR `skills-fallow` when it opens. This section is a placeholder so reviewers know what to look for in the implementation PR.

- **Scope of this change**: TBD — populated by `skills-fallow`
- **Default behavior after merge**: TBD (proposed: off until baseline locked; agents see the rule but it doesn't block)
- **Containment mechanism**: TBD (lefthook skip condition + `SUPERSET_FALLOW_ENABLED` env var)
- **One-line opt-out for an individual developer**: TBD
- **One-line rollback for the whole team**: TBD
- **What this change cannot break**: Typecheck (Sprint 1); lint (Biome at the file level); runtime code
- **Verification before merge**: see [`ROADMAP.md` § Sprint 03](./ROADMAP.md#sprint-03-fallow-dead-code-gate) Test Steps
