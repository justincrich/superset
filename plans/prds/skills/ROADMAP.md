...
**Sprints:** 8 (2 completed, 1 proposed, 5 future)
**Total Tasks:** ~55 (13 already shipped in Sprints 1–2, ~42 remaining)
**Current Sprint:** Sprint 3 — Fallow integration (next to plan)

## Sprint Sequence

| # | Sprint | Gate | Tasks | Dependencies | Status |
|---|--------|------|-------|--------------|--------|
| 1 | [Sprint 01: tsgo migration](#sprint-01-tsgo-migration) | `bun run typecheck` completes in ≤5s warm-cache; all 28 packages succeed | 8 | — | ✅ Completed |
| 2 | [Sprint 02: Agent-gated commit hooks](#sprint-02-agent-gated-commit-hooks) | Wrapped-agent commit runs strict lint+typecheck; plain-terminal commit skips clean | 5 | Sprint 01 | ✅ Completed |
...
**Status:** ✅ Completed (commit `3f534f7c8`)
...
**Sequence:** 2
**Timeline:** Phase 1 (tooling foundation)
**Status:** ✅ Completed (commit `692146e88`)
...
- **Sprints 1–2 are already implemented locally** in commits `3f534f7c8` and `692146e88` on the `skills` branch. Their PRs (`tooling/tsgo` and `tooling/commit-hooks`) will be created from those commits when the branch stack is split.
...
- **PR stacking**: each sprint becomes a PR stacked on its predecessor — `main ← tooling/tsgo ← tooling/commit-hooks ← tooling/fallow ← skills`. Each downstream branch inherits all upstream content.
